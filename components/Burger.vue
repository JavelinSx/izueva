...
<template>
  <div>
    ...
    <Transition name="fade">
      <div v-if="showAnimated" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[98]" @click="handleBurgerClose" />
    </Transition>
      ...
      <div class="flex justify-end">
        <button type="button"
          ...
          <div class="w-[28px] h-[20px] relative transition-all duration-500">
            <span class="block absolute h-[3px] w-full rounded-full left-0 bg-text transition-all duration-300"
      ...
      <div v-if="showAnimated"
        ...
        <div class="max-w-md mx-auto">
          <ul class="flex flex-col items-stretch gap-1 p-4">
            ...
            ]" :key="index" class="opacity-0 animate-slide-in-right"
              :style="{ animationDelay: `${index * 0.08}s`, animationFillMode: 'forwards' }">
              <LinkBurger :text="item.text" :anchor="item.anchor" :offset="-50" @close="handleBurgerClose" />
            </li>
            <li class="mt-4 opacity-0 animate-slide-in-right"
              :style="{ animationDelay: '0.48s', animationFillMode: 'forwards' }">
              <button
                class="w-full bg-gradient-to-r from-button/80 to-button/60 text-text text-base md:text-lg font-semibold rounded-xl py-3 px-6 transition-all duration-300 shadow-md hover:shadow-xl hover:from-button hover:to-button/80 hover:scale-[1.02] active:scale-[0.98]">
